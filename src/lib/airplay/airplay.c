...
conn_request(void *ptr, http_request_t *request, http_response_t **response)
{
	...
	if (strcmp(method, "OPTIONS") != 0 && strlen(airplay->password)) {
		const char *authorization;

		authorization = http_request_get_header(request, "Authorization");
		if (authorization) {
			logger_log(conn->airplay->logger, LOGGER_DEBUG, "Our nonce: %s", conn->nonce);
			logger_log(conn->airplay->logger, LOGGER_DEBUG, "Authorization: %s", authorization);
		}
		if (!digest_is_valid(realm, airplay->password, conn->nonce, method, http_request_get_url(request), authorization)) {
			char *authstr;
			int authstrlen;
			...
			free(authstr);
			logger_log(conn->airplay->logger, LOGGER_DEBUG, "Authentication unsuccessful, sending Unauthorized");
		} else {
			logger_log(conn->airplay->logger, LOGGER_DEBUG, "Authentication successful!");
		}
	}
	...
	if (!require_auth && challenge) {
		...
		rsakey_sign(airplay->rsakey, signature, sizeof(signature), challenge,
		            conn->local, conn->locallen, airplay->hwaddr, airplay->hwaddrlen);
		http_response_add_header(res, "Apple-Response", signature);

		logger_log(conn->airplay->logger, LOGGER_DEBUG, "Got challenge: %s", challenge);
		logger_log(conn->airplay->logger, LOGGER_DEBUG, "Got response: %s", signature);
	}
	...
	if (require_auth) {
		...
		if (data) {
			...
			logger_log(conn->airplay->logger, LOGGER_DEBUG, "connection: %s", remotestr);
			logger_log(conn->airplay->logger, LOGGER_DEBUG, "rtpmap: %s", rtpmapstr);
			logger_log(conn->airplay->logger, LOGGER_DEBUG, "fmtp: %s", fmtpstr);
			logger_log(conn->airplay->logger, LOGGER_DEBUG, "rsaaeskey: %s", aeskeystr);
			logger_log(conn->airplay->logger, LOGGER_DEBUG, "aesiv: %s", aesivstr);

			aeskeylen = rsakey_decrypt(airplay->rsakey, aeskey, sizeof(aeskey), aeskeystr);
			aesivlen = rsakey_parseiv(airplay->rsakey, aesiv, sizeof(aesiv), aesivstr);
			logger_log(conn->airplay->logger, LOGGER_DEBUG, "aeskeylen: %d", aeskeylen);
			logger_log(conn->airplay->logger, LOGGER_DEBUG, "aesivlen: %d", aesivlen);

			if (conn->airplay_rtp) {
				/* This should never happen */
				airplay_rtp_destroy(conn->airplay_rtp);
				conn->airplay_rtp = NULL;
			}
			conn->airplay_rtp = airplay_rtp_init(airplay->logger, &airplay->callbacks, remotestr, rtpmapstr, fmtpstr, aeskey, aesiv);
			if (!conn->airplay_rtp) {
				logger_log(conn->airplay->logger, LOGGER_ERR, "Error initializing the audio decoder");
		...
		if (dacp_id && active_remote_header) {
			logger_log(conn->airplay->logger, LOGGER_DEBUG, "DACP-ID: %s", dacp_id);
			logger_log(conn->airplay->logger, LOGGER_DEBUG, "Active-Remote: %s", active_remote_header);
			if (conn->airplay_rtp) {
			    airplay_rtp_remote_control_id(conn->airplay_rtp, dacp_id, active_remote_header);
			}
		}
		...
		logger_log(conn->airplay->logger, LOGGER_INFO, "Transport: %s", transport);
		...
		if (use_udp) {
			...
			if (original) {
				while ((tmpstr = utils_strsep(&current, ";")) != NULL) {
					...
					if (ret == 1) {
						logger_log(conn->airplay->logger, LOGGER_DEBUG, "Found remote control port: %hu", value);
						remote_cport = value;
					}
					ret = sscanf(tmpstr, "timing_port=%hu", &value);
					if (ret == 1) {
						logger_log(conn->airplay->logger, LOGGER_DEBUG, "Found remote timing port: %hu", value);
						remote_tport = value;
					}
				}
			}
		...
		if (conn->airplay_rtp) {
			airplay_rtp_start(conn->airplay_rtp, use_udp, remote_cport, remote_tport, &cport, &tport, &dport);
		} else {
			logger_log(conn->airplay->logger, LOGGER_ERR, "RAOP not initialized at SETUP, playing will fail!");
			http_response_set_disconnect(res, 1);
		}
		...
		logger_log(conn->airplay->logger, LOGGER_INFO, "Responding with %s", buffer);
		...
		if (!strcmp(content_type, "text/parameters")) {
			...
			while (current) {
				...
				next = strstr(current, "\r\n");
				if (next && !handled) {
					logger_log(conn->airplay->logger, LOGGER_WARNING,
		...
		if (!strcmp(content_type, "text/parameters")) {
			...
			if (data && datastr && conn->airplay_rtp) {
				memcpy(datastr, data, datalen);
				if (!strncmp(datastr, "volume: ", 8)) {
					float vol = 0.0;
					sscanf(datastr+8, "%f", &vol);
					airplay_rtp_set_volume(conn->airplay_rtp, vol);
				} else if (!strncmp(datastr, "progress: ", 10)) {
					unsigned int start, curr, end;
					sscanf(datastr+10, "%u/%u/%u", &start, &curr, &end);
					airplay_rtp_set_progress(conn->airplay_rtp, start, curr, end);
				}
			} else if (!conn->airplay_rtp) {
				logger_log(conn->airplay->logger, LOGGER_WARNING, "RAOP not initialized at SET_PARAMETER");
			}
			free(datastr);
		} else if (!strcmp(content_type, "image/jpeg") || !strcmp(content_type, "image/png")) {
			logger_log(conn->airplay->logger, LOGGER_INFO, "Got image data of %d bytes", datalen);
			if (conn->airplay_rtp) {
				airplay_rtp_set_coverart(conn->airplay_rtp, data, datalen);
			} else {
				logger_log(conn->airplay->logger, LOGGER_WARNING, "RAOP not initialized at SET_PARAMETER coverart");
			}
		} else if (!strcmp(content_type, "application/x-dmap-tagged")) {
			logger_log(conn->airplay->logger, LOGGER_INFO, "Got metadata of %d bytes", datalen);
			if (conn->airplay_rtp) {
				airplay_rtp_set_metadata(conn->airplay_rtp, data, datalen);
			} else {
				logger_log(conn->airplay->logger, LOGGER_WARNING, "RAOP not initialized at SET_PARAMETER metadata");
			}
		}
		...
		if (rtpinfo) {
			logger_log(conn->airplay->logger, LOGGER_INFO, "Flush with RTP-Info: %s", rtpinfo);
			if (!strncmp(rtpinfo, "seq=", 4)) {
				next_seq = strtol(rtpinfo+4, NULL, 10);
			}
		}
		if (conn->airplay_rtp) {
			airplay_rtp_flush(conn->airplay_rtp, next_seq);
		} else {
			logger_log(conn->airplay->logger, LOGGER_WARNING, "RAOP not initialized at FLUSH");
		}
	} else if (!strcmp(method, "TEARDOWN")) {
		http_response_add_header(res, "Connection", "close");
		if (conn->airplay_rtp) {
			/* Destroy our RTP session */
			airplay_rtp_stop(conn->airplay_rtp);
			airplay_rtp_destroy(conn->airplay_rtp);
			conn->airplay_rtp = NULL;
		}
	}
	...
	logger_log(conn->airplay->logger, LOGGER_DEBUG, "Handled request %s with URL %s", method, http_request_get_url(request));
	*response = res;
}
